...
export default function SchedulePage() {
  const [activeDay, setActiveDay] = useState<string>(() => {
    ...
    if (typeof window === "undefined") return "day-one"; // SSR fallback
    ...
    return active ? active.id : "day-one";
  });
  ...
  const currentDay = SCHEDULE.find(d => d.id === activeDay) || SCHEDULE.find(d => d.id === "day-one") || SCHEDULE[0];
  ...
  const groupedSlots = React.useMemo(() => {
    ...
    return Object.values(slotsByTime).sort((a, b) => a.timeStart.localeCompare(b.timeStart));
  }, [currentDay]);

  const isSlotNow = (slotTimeStart: string, nextSlotTimeStart: string | null, dayDateStr: string) => {
    if (!now) return false;
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <header className="pt-12 pb-6 px-4 text-center">
        <h1 className="text-4xl font-extrabold tracking-tight text-zinc-900 mb-2">Event Schedule</h1>
        <p className="text-zinc-500 font-medium">{process.env.NEXT_PUBLIC_FEST_NAME || "Tatvam 2026"}</p>
      </header>
        ...
        <div className="overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          <div className="flex w-max min-w-full sm:justify-center p-2">
            {SCHEDULE.map(day => (
              ...
              >
                <span className="font-bold text-sm whitespace-nowrap">{day.label}</span>
                <span className={`text-[10px] mt-0.5 ${activeDay === day.id ? "text-zinc-300" : "text-zinc-400"}`}>
                  {day.date}
                </span>
              </button>
            ))}
        ...
        <div className="relative">
          ...
          >
            {groupedSlots.map((slot, sIdx) => {
              ...
              const isNow = isSlotNow(slot.timeStart, nextSlot?.timeStart || null, currentDay.date);
                  ...
                  <div className="w-16 sm:w-20 shrink-0 text-right pr-4 pt-1 pb-4 relative z-10 flex flex-col">
                    {isNow && (
                      <span className="text-[10px] font-bold text-green-600 uppercase tracking-wider mb-1 animate-pulse flex items-center justify-end gap-1">
                        <span className="w-1.5 h-1.5 rounded-full bg-green-500 inline-block"></span> Now
                      </span>
                    )}
                    <span className={`text-xs sm:text-sm font-bold ${isNow ? 'text-green-700' : 'text-zinc-900'}`}>{startTime}</span>
                    <span className="text-[10px] sm:text-xs font-medium text-zinc-500">{endTime ? `– ${endTime}` : ""}</span>
                  </div>

                  {/* Dot */}
                  <div className={`relative z-10 shrink-0 w-4 h-4 rounded-full border-[3px] border-white ring-1 mt-1.5 mr-4 transition-colors shadow-sm ${isNow ? 'bg-green-500 ring-green-200' : 'bg-zinc-300 ring-zinc-200 group-hover:bg-zinc-500'}`}></div>

                  {/* Events column */}
                  <div className="flex-1 space-y-3 pb-2 pt-0.5 min-w-0">
                    {slot.events.map((event, eIdx) => {
                      const bgClass = event.color === "green" ? "bg-green-50/60 hover:bg-green-50 border-green-200" 
                                    : event.color === "yellow" ? "bg-amber-50/60 hover:bg-amber-50 border-amber-200" 
                                    : "bg-white hover:bg-zinc-50 border-zinc-200";
                      const borderLeftColor = event.color === "green" ? "border-l-green-400" 
                                            : event.color === "yellow" ? "border-l-amber-400" 
                                            : "border-l-zinc-300";
                      ...
                      return (
                        <div
                          key={eIdx}
                          onClick={() => isClickable && openModal(event, slot.time)}
                          className={`
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center text-xs font-medium text-zinc-500">
                                <span className="flex items-center gap-1 truncate">
                                  <svg className="w-3.5 h-3.5 shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                                  <span className="truncate">{event.venue}</span>
                                </span>
                              </div>
                            </div>
                            <div className="text-xl shrink-0 opacity-80" title={event.category || "General"}>
                          ...
                          <div className="mt-3 flex flex-wrap items-end justify-between gap-3">
                            ...
                            {isClickable && (
                              <span className="text-[10px] font-bold text-blue-600 uppercase tracking-wider flex items-center gap-0.5 group-hover/card:text-blue-700 shrink-0">
                                Register <svg className="w-3 h-3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>
                              </span>
                            )}
                          </div>
                        </div>
      ...
      <div className="max-w-3xl mx-auto px-4 pb-24 flex flex-col sm:flex-row gap-4 justify-center">
        <Link href="/events" className="flex items-center justify-center gap-2 w-full sm:w-auto px-6 py-3.5 bg-zinc-100 hover:bg-zinc-200 text-zinc-900 font-semibold rounded-xl transition-colors">
          Browse All Cultural Events <svg className="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
        </Link>
        <Link href="/sports" className="flex items-center justify-center gap-2 w-full sm:w-auto px-6 py-3.5 bg-zinc-100 hover:bg-zinc-200 text-zinc-900 font-semibold rounded-xl transition-colors">
          Browse Sports Events <svg className="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg>
        </Link>
      </div>
