...
export default function MerchCartPage() {
  const { merchCart, merchCartTotal, removeMerchUnit, clearMerchCart } = useMerchCart();
  ...
  const [finalUnits, setFinalUnits] = useState<any[]>([]);
  const [finalTotal, setFinalTotal] = useState(0);
  const [finalEmail, setFinalEmail] = useState("");
  const [finalPhone, setFinalPhone] = useState("");
  const [finalUtr, setFinalUtr] = useState("");

  const qrImagePath = process.env.NEXT_PUBLIC_MERCH_PAYMENT_QR_IMAGE_PATH || "/qr-code.webp";
  ...
  const validateField = (field: string, value: string) => {
    ...
    switch (field) {
      ...
      case "buyerEmail":
        ...
        break;
      case "buyerPhone":
        if (!/^\d{10,}$/.test(value.replace(/\D/g, ""))) error = "Valid phone number (min 10 digits) is required";
        break;
      case "utrNumber":
        if (!/^[A-Za-z0-9]{12,22}$/.test(value)) error = "UTR must be 12-22 alphanumeric characters";
        break;
    }
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    switch (name) {
      ...
      case "utrNumber": setUtrNumber(value); break;
    }
    if (errors[name]) {
  ...
  const handleScreenshotChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (!isValidType) {
        setErrors(prev => ({ ...prev, screenshot: "Must be a jpg, jpeg, or png image" }));
        setScreenshot(null);
        setScreenshotPreview(null);
        return;
      }
      if (!isSizeValid) {
        setErrors(prev => ({ ...prev, screenshot: "File must be smaller than 10MB" }));
        setScreenshot(null);
        setScreenshotPreview(null);
        return;
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (err.name === "AbortError") {
        setSubmitError("Upload is taking too long. Please check your connection and try again.");
      } else {
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-12 w-full">
        <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-200 text-center">
          ...
          <p className="text-gray-600 mb-8">Save this Order ID — our team will reference it when contacting you</p>
          
          <div className="bg-blue-50 border border-blue-100 rounded-lg p-6 mb-8">
            <p className="text-sm text-blue-600 font-semibold mb-1">Your Order ID</p>
            <p className="text-4xl font-mono font-bold text-blue-900">{orderId}</p>
          </div>
          ...
          <div className="text-left bg-gray-50 rounded-lg p-6 mb-8 border border-gray-200">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500 mt-1 space-y-0.5">
                      {Object.entries(unit.attributes).map(([key, val]) => (
                        <p key={key}>{key.charAt(0).toUpperCase() + key.slice(1)}: {val as string}</p>
                      ))}
                    </div>
                  </div>
            ...
            <div className="border-t mt-4 pt-4 text-sm text-gray-600">
              <p><strong>UTR:</strong> {finalUtr}</p>
            </div>
          </div>
          ...
          <div className="text-left space-y-4 mb-8">
            <p className="text-gray-700 flex items-center gap-2">
              ...
              A confirmation email has been sent to <strong>{finalEmail}</strong>
            </p>
            <div className="bg-yellow-50 border border-yellow-100 p-4 rounded-md text-yellow-800 text-sm">
              ...
              <p>All orders can be picked up from the registration desk. Our team will contact you at <strong>{finalPhone}</strong> to confirm your order details.</p>
            </div>
          </div>

          <Link href="/merch" className="inline-block px-8 py-3 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors">
            Shop More
          </Link>
        </div>
      </div>
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-16 w-full text-center">
        <div className="bg-gray-50 border border-gray-200 rounded-2xl p-12 max-w-lg mx-auto">
          ...
          <p className="text-gray-500 mb-6">Looks like you haven't added any merch yet.</p>
          <Link href="/merch" className="inline-block px-6 py-3 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors">
            Browse Merch
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 w-full">
      <div className="mb-8 flex items-center justify-between">
        ...
        {pageState === 2 && (
          <button onClick={() => setPageState(1)} className="text-sm text-blue-600 hover:text-blue-800 font-medium">
            &larr; Back to Cart
          </button>
        )}
      </div>
          ...
          {pageState === 1 ? (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="divide-y divide-gray-100">
                {merchCart.map((unit) => (
                  <div key={unit.unitId} className="p-6 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
                    <div>
                      <h3 className="font-bold text-gray-900 text-lg">{unit.itemName}</h3>
                      <div className="mt-2 space-y-1 text-sm text-gray-600">
                        {Object.entries(unit.attributes).map(([key, value]) => (
                          <p key={key}>
                            <span className="font-medium text-gray-700">{key.charAt(0).toUpperCase() + key.slice(1)}:</span> {value as string}
                          </p>
                        ))}
                      </div>
                    </div>
                    <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto gap-4">
                      <span className="font-bold text-gray-900 text-xl">₹{unit.price}</span>
                      <button
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      onBlur={handleBlur}
                      className={`w-full p-2.5 border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.buyerName ? 'border-red-500' : 'border-gray-300'}`}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      onBlur={handleBlur}
                      className={`w-full p-2.5 border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.buyerEmail ? 'border-red-500' : 'border-gray-300'}`}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      onBlur={handleBlur}
                      className={`w-full p-2.5 border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.buyerPhone ? 'border-red-500' : 'border-gray-300'}`}
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div className="bg-blue-50 border border-blue-100 rounded-lg p-6 mb-6 text-center">
                  <p className="text-lg text-blue-900 font-medium mb-4">Pay <span className="text-2xl font-bold">₹{merchCartTotal}</span> via UPI</p>
                  ...
                  <p className="text-sm text-blue-700">Scan this QR code to complete your payment.</p>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">Accepts JPG, PNG (Max 10MB)</p>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      onBlur={handleBlur}
                      className={`w-full p-2.5 border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 uppercase ${errors.utrNumber ? 'border-red-500' : 'border-gray-300'}`}
                      placeholder="e.g. 123456789012"
                    />
                    {errors.utrNumber && <p className="text-red-500 text-xs mt-1">{errors.utrNumber}</p>}
                  </div>
                </div>
              </div>
            </div>
